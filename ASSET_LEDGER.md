...
clearance. Given the no-import procedural pipeline, the sole known commercial
redistribution blocker remaining in this asset set is RockerKaki's unresolved
remove.bg step, documented in `ASSETS.md`.
...
RockerKaki remains a project-owner-supplied AI-generated derivative.  Its
source chain and the unresolved remove.bg account/plan are documented in
`ASSETS.md`; this workstream did not rewrite the hero or claim that uncertain
step is commercially cleared.  A release still needs either qualifying
remove.bg account evidence or a clean owner-controlled replacement.
