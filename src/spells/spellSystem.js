...
export class SpellSystem {
    ...
    constructor(scene, sky, shadows, terrain, controller, figure, rig, spray) {
        ...
        */
        this._consumers = [];
        /** Parallel activation flags; hero switches happen outside the hot path. */
        this._consumerEnabled = [];
        this._figureHandsEnabled = true;
    ...
    addConsumers(...mats) {
        for (let i = 0; i < mats.length; i++) {
            if (mats[i]) {
                this._consumers.push(mats[i]);
                this._consumerEnabled.push(true);
            }
        }
    }

    /**
     * Toggle an already-registered group without rebuilding the consumer list.
     * Called only when the selected hero changes, never per frame.
     * @param {import("@babylonjs/core/Materials/shaderMaterial").ShaderMaterial[]} mats
     * @param {boolean} enabled
     */
    setConsumersEnabled(mats, enabled) {
        for (let i = 0; i < this._consumers.length; i++) {
            const consumer = this._consumers[i];
            for (let k = 0; k < mats.length; k++) {
                if (consumer === mats[k]) {
                    this._consumerEnabled[i] = !!enabled;
                    break;
                }
            }
        }
    ...
    /** Use planted procedural hands only while the Snowbound figure is active. */
    setFigureHandsEnabled(enabled) {
        this._figureHandsEnabled = !!enabled;
    }
    ...
    _handPosition(which, out, off) {
        ...
        if (fig && this._figureHandsEnabled) {
     ...
     * without synthesising a key event. `KAKISNOW.spells` is the console handle.
     *
