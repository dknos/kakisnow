...
function countTriangles(fillMode, count, instancesCount) {
    const instances = instancesCount || 1;
    if (fillMode === 0) return ((count / 3) | 0) * instances;
    if (fillMode === 7 || fillMode === 8) {
        return Math.max(0, count - 2) * instances;
    }
    return 0;
}
...
export function installDrawCounter(engine) {
    ...
    engine.drawElementsType = function (a, b, c, d) {
        drawAccum++;
        triangleAccum += countTriangles(a, c, d);
        elements(a, b, c, d);
    };
...
/** Latch full submitted work, including shadows, prepasses, and post passes. */
export function endFrameDraws() {
    stats.drawCalls = drawAccum;
    stats.triangles = triangleAccum;
    drawAccum = 0;
    triangleAccum = 0;
}
