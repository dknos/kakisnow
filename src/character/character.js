...
export class Character {
    ...
    constructor(scene, terrain, sky, shadows, controller) {
        ...
        // Fixed arrays created once: these lists are traversed every frame.
        this._surfaceMats = [this.bodyMat, this.clothMat, this.furMat];
        this._fabricMats = [this.bodyMat, this.clothMat];
        this.lightConsumers = this._fabricMats;
    ...
    _pushUniforms() {
        ...
        const fabricMats = this._fabricMats;
        for (let i = 0; i < fabricMats.length; i++) {
            const m = fabricMats[i];
