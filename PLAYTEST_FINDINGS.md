...
| 4 | P1 | Big Air must retain and present one authoritative completed flight per vehicle without trusting malformed saves or incomplete attempts. | Fresh accepted flights report 2.53–2.54 s, 49.2–49.3 m, 18.5–18.7 m clearance and distinct vehicle state. Exact-event/completed-run PB trust, corrupt key/payload rejection, legacy compatibility, assembly integration, abandon gates, first-PB, repeat delta, and ordinary-event isolation all pass independent checks. | Keep controller-authoritative telemetry and key-authoritative defensive reads; final camera/showreel/integration evidence confirms the presentation contract. | Classic and rocket scripted runs both take off across the seed set, land controllably, and report internally consistent flight telemetry and PB state. | P1 FIXED · FINAL INTEGRATION PASS |
...
| 7 | P1 | Central ingredient, burger, rocket-chair, and dressing assets need trustworthy runtime provenance, reproducible source, contract compatibility, and focal visual quality; RockerKaki separately retains an uncertain remove.bg step. | F4 independently promoted all 13 deterministic originals after two byte-identical clean generations, 13/13 Khronos validation, 52 four-view renders, exact rocket anchors, and real Summit 16:9/21:9 plus rocket WebGPU runs. F5 proved default validation cannot overwrite the immutable historical audit and rechecked all 13 manifest hashes. | Keep the accepted generated-source/runtime manifest and guarded validator. Retain the explicit noncommercial limitation until the hero remove.bg chain is repaired or the owner supplies account-tier evidence. | Every active central runtime model has source, terms record, hashes, modifications, date, use, contract validation, and runtime pixels; the historical audit stays byte-identical. | CENTRAL ASSETS FIXED · ROCKERKAKI RIGHTS BLOCKER OPEN |
| 8 | P1 | A validation or browser failure must be incapable of publishing Pages, and hosted software smoke must not masquerade as hardware certification. | Independent H2 recheck passed 4/4 release tests, normal production-preview boot with zero console/page/request/HTTP errors, and two fail-closed synthetic fatal-error runs. Known authored WebGPU-unavailable messages reach the product error state; unrelated device/buffer invariants exit 1. The combined report is always uploaded, candidate ledger hashes are cross-checked against bytes, branch pushes cannot deploy, and Pages revalidates the exact successful main SHA. | Keep the accepted fail-closed mechanism and pair hosted boot evidence with the existing discrete NVIDIA WebGPU release matrix. The strict validator intentionally keeps the single RockerKaki/remove.bg hero-provenance blocker visible. | Intentionally failing validation prevents deploy; hosted CI records source/browser evidence; accepted Windows release evidence covers all events/WebGPU without being mislabelled as hosted GPU certification. | P1 ACCEPTED · RELEASE BLOCKERS VISIBLE |
...
- Clean `npm ci`, 90-test baseline suite, and production build pass at the
  ...
  157 tests; the latest converged package passes 182/182 tests and a production
...
182/182 tests, the production build, all twelve production event completions,
and 600/600 real-heightfield placement seeds. The final 74.560-second showreel
...
speaker/headphone listening remain device gates. Strict release validation is
intentionally still a failure for exactly the RockerKaki/remove.bg
hero-provenance record, so this package is not commercially READY.
